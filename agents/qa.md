...
아래 경우 추적 이슈 생성 (`mcp__github__create_issue` 또는 tracker CLI) 호출 금지:
- **관련 모듈/파일 = 0** → `SCOPE_ESCALATE` 마커 출력 후 중단
- `DUPLICATE_OF`로 기존 이슈와 중복 판정
- 프롬프트에 `issue: #N` 또는 `issue: LOCAL-N` (N ≠ 0)으로 기존 추적 ID가 이미 전달된 경우

### MCP 미가용 폴백 (tracker CLI)

`mcp__github__create_issue` 호출이 실패하거나 (네트워크 차단·인증 오류·MCP 서버 미연결), GitHub repo 가 연결되지 않은 환경에선 `harness/tracker.py` LocalBackend 로 폴백한다:
...
# stdout: "#42" (gh 가용) 또는 "LOCAL-42" (gh 미가용)
```

`harness/tracker.py` 가 환경에 따라 GitHub / Local 중 가용 백엔드를 자동 선택한다. 출력 ID 는 양쪽 형식 모두 다운스트림(architect/engineer 호출, agent-gate.py) 에서 수용된다.

판단 흐름:
1. MCP 도구(`mcp__github__create_issue`) 시도 — 성공하면 그대로 사용
2. MCP 실패 시 Bash + tracker CLI 폴백
3. Bash 도 실패하면 `EXTERNAL_TRACKER_NEEDED` 마커 출력 후 메인 Claude 에 위임
...
- **Bash 도구는 추적 ID 발급 폴백 전용** — 분석·코드 탐색·일반 명령어 실행 금지. Bash 는 오직 `python3 -m harness.tracker create-issue|comment|update-issue` (MCP 폴백 경로) 한정으로 사용. 나머지 분석은 Read/Glob/Grep.
