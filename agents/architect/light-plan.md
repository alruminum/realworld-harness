...
마커는 **출력 마지막 줄에 정형(`---MARKER:LIGHT_PLAN_READY---`)** 으로 둔다. 평이 단어 `LIGHT_PLAN_READY` 만 본문에 적는 형태는 `parse_marker` 의 fallback 으로는 잡히지만, 출력 도중 끊김·punctuation 변형(`LIGHT_PLAN_READY:`)에 취약해 `SPEC_GAP_ESCALATE` 위험이 있다.
