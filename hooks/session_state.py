...
def current_session_id(project_root: Optional[Path] = None) -> str:
    ...
    if sid:
        return sid
    # 글로벌 폴백 — RWHarness dogfooding 등 프로젝트 pointer 미생성 환경 대응
    return _read_global_session_pointer_safely(project_root)


def _read_global_session_pointer_safely(project_root: Optional[Path] = None) -> str:
    """글로벌 ~/.claude pointer 폴백 — 모든 가드 통과 시에만 sid 반환.

    자기참조 회피: project_root 가 ~/.claude 이면 글로벌 폴백 skip (무한 루프 방지).
    """
    try:
        home = Path.home()
        # 자기참조 회피 — project_root 가 ~/.claude 이면 글로벌 폴백 skip
        if project_root is not None:
            try:
                if Path(project_root).resolve() == (home / ".claude").resolve():
                    return ""
            except (OSError, ValueError):
                return ""
        home_root = home / ".claude" / "harness-state"
