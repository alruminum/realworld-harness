...
- 후속 sub-commit:
  ...
  - **[3.3] (완료)** `docs/harness-architecture.md` 본문 잔존 `~/.claude/*` 경로를 `${CLAUDE_PLUGIN_ROOT}/...` 로 적응 — §1 시스템 구성요소 표 (Hooks/Harness Core/Agents/Skills 4행) / §2.1 훅 등록 안내 / §5.3 setup-project.sh 호출 / §6 changelog.md 위치 / 마지막 노트 정리
- 다음 sub-commit 후보 ([3.4]+):
  - GitHub Actions workflow (PR 단계 자동 게이트)
  - clean install smoke test (별도 머신/컨테이너)
  - v1.0.0 태그 + 마켓플레이스 PR + public 전환
