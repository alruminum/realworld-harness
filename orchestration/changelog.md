...
| `HARNESS-CHG-20260428-42` | 2026-04-28 | infra | [42.1] plan-reviewer 검토 범위 경계 강화 — jajang #133 reviewer 가 PRD 외 파일(docs/impl/06, docs/impl/07, batch.md, stories.md, audio-engine.md) 26회 Glob/Grep 으로 헤매다 "impl 계획 부재" 잘못된 CHANGES_REQUESTED 발행한 사고 차단. `agents/plan-reviewer.md` 최상단에 🔴 검토 범위 경계 섹션 신설 — 허용/금지 파일 enumeration + scope drift 자기규율(이슈 본문 파일명 환각 차단, 도구 호출 5회 budget). 9개 금지 경로(docs/impl/, docs/milestones/, stories.md, batch.md, audio-engine.md, voice-pipeline.md, apps/, src/, trd.md) 박제. `tests/pytest/test_plan_reviewer_scope.py` 신규 (5클래스 19TC — 섹션 존재 / 금지 파일 9건 / 허용 파일 5건 / 자기규율 / next-stage 책임). pytest 19/19 신규 + 282/282 전체 PASS. | — |
| `HARNESS-CHG-20260429-43` | 2026-04-29 | infra | [43.1] plan-reviewer 마커 미감지 fail-safe 분기 metadata 저장 추가 — HARNESS-CHG-41 (#45) 에서 정식 PASS / CHANGES_REQUESTED 분기에 metadata 저장 추가했지만, 마커 미감지 fail-safe 분기 (line 243-251) 는 누락. plan-reviewer 가 마커 못 뽑은 케이스에도 PRD 는 이미 작성된 상태이므로 체크포인트 보존해야 재시도 시 planner 처음부터 안 돌음. fail-safe return 직전에 save_plan_checkpoint 호출 1줄 추가. 전체 282/282 PASS. | — |
| `HARNESS-CHG-20260429-44` | 2026-04-29 | infra | [44.1] plan_loop hash 기반 게이트 무효화 — plan-reviewer 와 ux-validation 매번 재실행되던 비용 hole + ux-architect 가 PRD 변경 후에도 옛 ux-flow 보고 stale 사용하던 hole 동시 차단. `compute_file_hash()` (sha256) + `load_plan_checkpoint()` 헬퍼 신설. `save_plan_checkpoint()` 시그니처 확장 — keyword-only `plan_review_passed_for` / `ux_validation_passed_for` / `merge=True`. metadata schema 확장: `prd_hash`, `ux_flow_hash`, `plan_review_passed_for`, `ux_validation_passed_for` 키 자동 기록. plan-reviewer 진입 분기에 hash 비교 skip + PASS 직후 hash 저장. ux-validation 동일 패턴. ux-architect skip 도 PRD hash 일치 시에만 (PRD 변경 시 stale 보호). `tests/pytest/test_plan_checkpoint.py` 확장 (10클래스 14TC — REQ-007~010 hash helper / plan_review_passed_for 무효화 / ux_validation_passed_for / merge=True 보존). pytest 14/14 + 289/289 전체 PASS. | — |
| `HARNESS-CHG-20260429-45` | 2026-04-29 | infra | [45.1] plan-reviewer 마커 키워드 정합 + alias 보강 — jajang #133 reviewer 가 정식 PASS 결론 후 `---MARKER:LGTM---` (pr-reviewer 마커) emit → executor PLAN_REVIEW_PASS 정확 매칭 실패 → 자동 CHANGES_REQUESTED 처리되던 사고 차단. 두 layer 동시 보강 (defense in depth). (1) `agents/plan-reviewer.md` 출력 형식 §최상단에 🔴 마커 키워드 절대 규칙 신설 — 정식 2개(PASS/CHANGES_REQUESTED) 명시 + 5개 금지 변형(LGTM/APPROVE/APPROVED/OK/PASS bare) 박제. (2) `harness/core.py` MARKER_ALIASES 에 PLAN_REVIEW_LGTM/OK/APPROVE/APPROVED → PLAN_REVIEW_PASS, PLAN_REVIEW_REJECT/REJECTED/FAIL → PLAN_REVIEW_CHANGES_REQUESTED 7개 alias 추가. bare LGTM 은 의도적 미alias 유지 (pr-reviewer 충돌 방지). `tests/pytest/test_tracker.py` 확장 (5 TC — PLAN_REVIEW alias 4종 + bare LGTM 미alias 검증), `tests/pytest/test_plan_reviewer_marker.py` 신규 (4클래스 9TC — 절대 규칙 섹션 / 정식 마커 / 금지 변형 / alias 인프라). pytest 16/16 신규 + 305/305 전체 PASS. | — |
| `HARNESS-CHG-20260429-46` | 2026-04-29 | infra | [46.1] bare LGTM alias — HARNESS-CHG-45 후속 hot-fix. agent.md 강화 후에도 jajang `run_20260429_001354` 에서 plan-reviewer 가 다시 bare `---MARKER:LGTM---` emit 발생. parse_marker 의 1차 canonical 매칭이 alias 보다 우선이라 pr-reviewer 호출(expected_set 에 LGTM 포함)에선 1차 매치 → alias 우회됨 → 충돌 없음. 따라서 bare LGTM 도 안전하게 alias 가능. MARKER_ALIASES 에 `"LGTM": "PLAN_REVIEW_PASS"` 추가. pr-reviewer 컨텍스트 회귀 보호 TC 동시 추가. pytest 306/306 전체 PASS. | — |
